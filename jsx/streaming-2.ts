...
//const { childrenToString } = require('./index');
...
function Suspense(props: JSX.JSXProps & {rid: string | number}) {
  ...
  const fallback = childrenToString([props.fallback]);
  ...
  return fallback.then(function resolveCallback(resolved) {
    return '<div id="B:' + run + '" data-sf>' + resolved[0] + '</div>';
  });
  ...
  function writeStreamTemplate(result) {
    ...
    stream.write(
      // prettier-ignore
      '<template id="N:' + run + '" data-sr>' + result[0] + '</template><script id="S:' + run + '" data-ss>$KITA_RC(' + run + ')</script>'
    );
  }
}
...
function pipeHtml(html, stream, rid) {
  ...
  html
    .then(function writeStreamHtml(html) {
      console.log({ html, stream })
      
      if (!stream  || stream._readableState.ended) {
        return 
      }
...
function renderToStream(factory: (props: JSX.JSXProps) => JSX.Element) {
  var rid = SUSPENSE_ROOT.requestCounter++;
  ...
  if (SUSPENSE_ROOT.requests.has(rid)) {
  ...
  try {
    // Loads the template
    const html = factory(rid).toString();
